...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        # self.loss_fn = PonderatedLoss().cuda()
        
    def training_step(self, data, loss, device):
        output = self(data['leftEye'].to(device), 
                     data['rightEye'].to(device),
                     data['face'].to(device),
                     data['rects'].to(device))

        loss_value_reg = loss(output[0], data['label'].to(device)) * 4
        loss_value_class = nn.CrossEntropyLoss()(output[1], data['class_label'].to(device))
        
        return 0.2 * loss_value_class + loss_value_reg
    ...
    def validation_step(self, data, device):
        ...
        return output[0], labels, resolution, data['name']
    ...
    def forward(self, eyesLeft, eyesRight, faces, rects):
        ...
        out_reg = self.FCLayer(f_out)
        out_class = self.classificator(f_out)
